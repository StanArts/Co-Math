...
protocol OnlineGameCellDelegate: class {
  func didChooseAnswer(btnIndex: Int)
}
class OnlineGameCell: UICollectionViewCell {
  ...
  func setupViews() {
    ...
    QuestionLabel.heightAnchor.constraint(equalTo: imgView.widthAnchor).isActive = true
    
    addSubview(imgView)
    imgView.topAnchor.constraint(equalTo: imgView.bottomAnchor).isActive = true
    imgView.leftAnchor.constraint(equalTo: self.leftAnchor, constant: 12).isActive = true
    imgView.rightAnchor.constraint(equalTo: self.rightAnchor, constant: -12).isActive = true
